...
const BurnPanel = ({ token,ptokens, type,dynasetid }) => {
  ...
  const [ratios, setRatios] = useState(["00000000000000000000000","00000000000000000000000","00000000000000000000000"]);
  ...
  //  const DAI = new Token(ChainId.ROPSTEN, dynasetid, 18);
    ...
    const results = await Promise.all(ptokens.map(async (i) => {
      ...
      return {symbol:coin.symbol, ratio:ethers.BigNumber.from(token).toString(),address:coin.id};
    }));
